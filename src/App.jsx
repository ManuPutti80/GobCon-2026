...
import { X, Users, MapPin, LogOut, Trash2, Search, Calendar, HelpCircle, Box, Clock, CheckCircle, MessageCircle, Send, ExternalLink, Filter } from 'lucide-react'
...
function App() {
  ...
  useEffect(() => {
    ...
    if (isChatOpen && currentChatMatch) {
        ...
        chatChannel = supabase
            ...
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `match_id=eq.${currentChatMatch.id}` }, (payload) => {
  ...
  const fetchMatches = async () => {
    ...
    if (!error) {
        const sorted = data.sort((a, b) => {
            const isTomorrowA = a.match_time.toLowerCase().includes('domani');
            const isTomorrowB = b.match_time.toLowerCase().includes('domani');
  ...
  const openCreationModal = (mode) => {
      setModalMode(mode); setGameName(''); setSuggestions([]); setNotes(''); setTableNr(''); 
      generateTimeOptions();
      const now = new Date();
      setWhenTime(now.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' }));
      setIsModalOpen(true)
  }
  ...
  const createMatch = async () => {
    ...
    const { error } = await supabase.from('matches').insert([{
      ...
      max_players: maxPlayers, current_players: 1, players_list: [currentUser],
      status: 'OPEN', bgg_id: notes || null, match_type: modalMode,
      match_time: whenTime, is_archived: false
    }])
  ...
  const leaveMatch = async (match) => {
    ...
    await supabase.from('matches').update({ players_list: newPlayers, current_players: newCount, status: 'OPEN' }).eq('id', match.id)
  }

  const finishMatch = async (matchId) => {
    if(!confirm("La partita è finita o il tavolo è stato annullato?")) return;
    setMatches(matches.filter(m => m.id !== matchId));
    await supabase.from('matches').update({ is_archived: true }).eq('id', matchId)
  }

  const deleteMatch = async (matchId) => {
      if(!confirm("Vuoi cancellare definitivamente (errore inserimento)?")) return;
      setMatches(matches.filter(m => m.id !== matchId));
  ...
  const filteredMatches = matches.filter(match => {
    ...
    // Se il toggle "I Miei Tavoli" è attivo, mostra solo quelli dove ci sono io
    if (showMyMatchesOnly) {
        return matchesSearch && match.players_list?.includes(currentUser);
    }
    ...
    <div className="min-h-screen bg-gray-50 pb-32 font-sans">
      ...
      <div className="bg-green-700 text-white p-3 shadow-md sticky top-0 z-20">
        <div className="flex items-center gap-3 mb-2">
            ...
            <div>
                ...
                <div className="flex items-center gap-1 text-green-100 text-[10px] font-medium uppercase tracking-wide"><Calendar size={10}/> Arezzo 27 Feb - 1 Mar</div>
            </div>
        </div>
      ...
      <div className="bg-white px-4 py-2 border-b border-gray-100 flex justify-between items-center text-xs text-gray-500">
        ...
        <button 
            ...
            className={`flex items-center gap-1 px-3 py-1 rounded-full border transition-all ${showMyMatchesOnly ? 'bg-green-600 text-white border-green-600 shadow-md' : 'bg-gray-100 text-gray-500 border-gray-200'}`}
        >
            <Filter size={12} />
            <span className="font-bold">I Miei Tavoli</span>
        </button>
      </div>
            ...
            <div key={match.id} className={`${bgColor} p-4 rounded-xl shadow-sm border border-gray-100 ${cardBorder} relative overflow-hidden flex flex-col gap-2 animate-fade-in`}>
              <div className="flex justify-between items-start">
                 <div>
                    <div className="flex items-center gap-2 flex-wrap">
                        {/* LINK BGG */}
                        <a 
                            href={`https://boardgamegeek.com/geeksearch.php?action=search&objecttype=boardgame&q=${encodeURIComponent(match.game_name)}`} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="text-lg font-bold text-gray-800 leading-tight hover:text-green-700 hover:underline flex items-center gap-1"
                        >
                            {match.game_name} <ExternalLink size={12} className="text-gray-400"/>
                        </a>

                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase flex items-center gap-1 bg-blue-100 text-blue-700`}>
                            <Clock size={10}/> {match.match_time}
                        </span>
                    </div>
                 ...
                 <div className="flex flex-col gap-2">
                     {isHost && ( <>
                         <button onClick={() => finishMatch(match.id)} className="text-green-600 bg-green-50 hover:bg-green-100 p-1.5 rounded-lg border border-green-200"><CheckCircle size={20}/></button>
                         <button onClick={() => deleteMatch(match.id)} className="text-gray-300 hover:text-red-500 p-1"><Trash2 size={14}/></button>
                     </> )}
                 </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                  {match.players_list?.map((p, i) => (
                      <span key={i} className={`px-2 py-0.5 rounded text-[10px] border ${p===match.host_name ? (isRequest ? 'bg-orange-100 border-orange-200 text-orange-800' : 'bg-green-100 border-green-200 text-green-800') : 'bg-white border-gray-200 text-gray-600'}`}>{p}</span>
                  ))}
              </div>
              <div className="mt-2 flex gap-2">
                <div className="flex-1">
                    {amIIn ? ( <button onClick={()=>leaveMatch(match)} className="w-full py-2.5 bg-white border border-red-200 text-red-600 font-bold rounded-lg text-xs hover:bg-red-50 flex justify-center gap-2 items-center"><LogOut size={14}/> {isRequest ? 'Annulla' : 'Lascia'}</button>
                    ) : ( !isFull ? ( <button onClick={()=>joinMatch(match)} className={`w-full py-2.5 text-white font-bold rounded-lg text-xs shadow-md active:scale-95 ${isRequest ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700'}`}>{isRequest ? 'Ce l\'ho io!' : 'Mi unisco'}</button>
                ...
                <button onClick={() => openChat(match)} className="w-12 flex items-center justify-center bg-blue-50 text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-100 active:scale-95 transition-colors relative">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white via-white to-transparent pt-10 flex gap-3 justify-center z-20">
          <button onClick={() => openCreationModal('REQUEST')} className="flex-1 max-w-[160px] bg-orange-500 text-white p-3 rounded-xl shadow-lg hover:bg-orange-600 active:scale-95 transition-transform flex flex-col items-center justify-center gap-1"><HelpCircle size={24} /><span className="text-xs font-bold">Richiedi Gioco</span></button>
          <button onClick={() => openCreationModal('OFFER')} className="flex-1 max-w-[160px] bg-green-700 text-white p-3 rounded-xl shadow-lg hover:bg-green-800 active:scale-95 transition-transform flex flex-col items-center justify-center gap-1"><Box size={24} /><span className="text-xs font-bold">Proponi Tavolo</span></button>
      </div>

      {/* MODALI (Creazione / Chat) sono qui sotto... (Codice invariato per brevità visiva, ma funzionalmente identico) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 h-[85vh] sm:h-auto shadow-2xl flex flex-col animate-slide-up">
            <div className="flex justify-between items-center mb-4">
              <h2 className={`text-xl font-bold ${modalMode === 'REQUEST' ? 'text-orange-600' : 'text-green-700'}`}>{modalMode === 'REQUEST' ? 'Cerco Gioco' : 'Proponi Tavolo'}</h2>
            ...
            <div className="space-y-5 flex-1 overflow-y-auto pb-4">
              <div className="relative">
                <label className="block text-sm font-bold text-gray-700 mb-2">{modalMode === 'REQUEST' ? 'Che gioco vorresti?' : 'A cosa giochiamo?'}</label>
                <input type="text" placeholder="Scrivi..." className={`w-full p-4 bg-gray-50 border rounded-xl text-lg outline-none focus:ring-2 ${modalMode === 'REQUEST' ? 'focus:ring-orange-400' : 'focus:ring-green-500'}`} value={gameName} onChange={(e) => setGameName(e.target.value)} autoFocus />
                {suggestions.length > 0 && <ul className="absolute z-50 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-48 overflow-y-auto">{suggestions.map((g, i) => <li key={i} onClick={() => selectSuggestion(g)} className="p-3 border-b hover:bg-green-50 cursor-pointer">{g}</li>)}</ul>}
              </div>
              <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Quando? (Orario modificabile)</label>
                  <input type="text" value={whenTime} onChange={(e) => setWhenTime(e.target.value)} className="w-full p-3 mb-3 bg-white border border-green-500 text-green-800 font-bold rounded-xl text-center shadow-sm"/>
                  <div className="grid grid-cols-3 gap-2">{dynamicTimeOptions.map((opt, i) => (<button key={i} onClick={() => setWhenTime(opt.value)} className="p-2 bg-gray-100 border border-gray-200 rounded-lg text-xs font-medium hover:bg-green-100 hover:border-green-300 transition-colors"><div className="font-bold">{opt.label}</div><div className="text-[10px] text-gray-500">{opt.value.includes(' ') ? '' : opt.value}</div></button>))}</div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                   <label className="block text-sm font-bold text-gray-700 mb-2">Giocatori Max</label>
                   <div className="flex items-center border border-gray-200 rounded-xl bg-gray-50"><button onClick={() => setMaxPlayers(Math.max(2, maxPlayers-1))} className="px-4 py-3 hover:bg-gray-200 text-lg font-bold">-</button><input type="number" value={maxPlayers} readOnly className="w-full text-center bg-transparent font-bold"/><button onClick={() => setMaxPlayers(maxPlayers+1)} className="px-4 py-3 hover:bg-gray-200 text-lg font-bold">+</button></div>
                 </div>
                 <div>
                   <label className="block text-sm font-bold text-gray-700 mb-2">Tavolo</label>
                   {modalMode === 'REQUEST' ? <div className="w-full p-3.5 bg-orange-50 border border-orange-200 rounded-xl text-orange-800 font-bold text-center text-sm">CERCASI</div> : <select className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl font-medium h-full" value={tableNr} onChange={(e) => setTableNr(e.target.value)}><option value="">Da definire</option>{[...Array(30)].map((_, i) => <option key={i} value={i+1}>Tavolo {i+1}</option>)}</select>}
                 </div>
              </div>
              ...
              <div className="pt-2 mt-auto"><button onClick={createMatch} className={`w-full text-white font-bold py-4 rounded-2xl shadow-xl text-lg active:scale-95 ${modalMode === 'REQUEST' ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-700 hover:bg-green-800'}`}>{modalMode === 'REQUEST' ? 'Pubblica' : 'Crea Tavolo'}</button></div>
            </div>
          </div>
        </div>
      )}
      ...
      {isChatOpen && currentChatMatch && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm animate-fade-in">
              <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl h-[85vh] sm:h-[600px] shadow-2xl flex flex-col overflow-hidden animate-slide-up">
                  <div className="bg-gray-50 p-4 border-b flex justify-between items-center shadow-sm z-10">
                      <div><h3 className="font-bold text-gray-800 text-lg leading-none">{currentChatMatch.game_name}</h3><span className="text-xs text-gray-500">Chat del tavolo</span></div>
                      <button onClick={() => setIsChatOpen(false)} className="p-2 bg-white border border-gray-200 rounded-full hover:bg-gray-100"><X size={20}/></button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#f0f2f5]">
                      {chatMessages.length === 0 && <div className="text-center text-gray-400 text-sm mt-10">Non ci sono ancora messaggi.<br/>Scrivi il primo! 👋</div>}
                      {chatMessages.map((msg) => { const isMe = msg.user_name === currentUser; return ( <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}><div className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm ${isMe ? 'bg-green-600 text-white rounded-tr-none' : 'bg-white text-gray-800 border border-gray-200 rounded-tl-none'}`}>{!isMe && <div className="text-[10px] font-bold text-gray-400 mb-1">{msg.user_name}</div>}{msg.content}</div><span className="text-[10px] text-gray-400 mt-1 px-1">{new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span></div> ) })}
                      <div ref={messagesEndRef} />
                  </div>
                  <div className="p-3 bg-white border-t flex gap-2 items-center">
                      <input type="text" placeholder="Scrivi..." className="flex-1 p-3 bg-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-green-500 text-sm" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} />
                      <button onClick={sendMessage} disabled={!newMessage.trim()} className="p-3 bg-green-600 text-white rounded-xl shadow-md disabled:opacity-50 hover:bg-green-700 active:scale-95 transition-transform"><Send size={20}/></button>
                  </div>
              </div>
          </div>
      )}
